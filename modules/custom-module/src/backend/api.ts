...
export default async (bp: typeof sdk) => {
   ...
   * Your API will be available at `http://localhost:3000/api/v1/bots/BOT_NAME/mod/custom-module`
   * Just replace BOT_NAME by your bot ID
   */
  const router = bp.http.createRouterForBot('custom-module')

  // Link to access this route: http://localhost:3000/api/v1/bots/BOT_NAME/mod/custom-module/my-first-route
  router.get('/my-first-route', async (req, res) => {
     ...
     */
    const config = await bp.config.getModuleConfigForBot('custom-module', botId)

    res.sendStatus(200)
  })

  router.get('/foo', (req, res) => {
    res.send({ name: 'hisagi' })
  })
}
